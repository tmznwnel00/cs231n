...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    dx1, dprev_h, dWx, dWh, db = rnn_step_backward(dh[:, T - 1, :], cache[T - 1])
    D = dx1.shape[1]

    dx = np.zeros((N, T, D))
    dx[:, T - 1, :] = dx1
    for i in range(T - 2, -1, -1):
      dx2, dprev_h2, dWx2, dWh2, db2 = rnn_step_backward(dh[:, i, :] + dprev_h, cache[i])
      dx[:, i, :] += dx2
      dprev_h = dprev_h2
      dWx += dWx2
      dWh += dWh2
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
      xi = x[:, i, :].reshape(N, D)
      next_h, next_c, cache_i = lstm_step_forward(xi, prev_h, prev_c, Wx, Wh, b)
      h[:, i, :] = next_h
